  ...
  methods: {
    ...
    ON_SUGGESTION_ADD({ suggestion }) {
      ...
      if (this.removedSuggestions.find((a) => a.name == suggestion.name)) {
        ...
        this.removedSuggestions.splice(
          this.removedSuggestions.indexOf(suggestion.name),
          1
        );
      }

      console.log(
        "Adding Command Suggestion: " + suggestion.name + " " + suggestion.help
      );
    ...
    send(e) {
      if (this.message !== "") {
        post(
          `https://${GetParentResourceName()}/chatResult`,
          JSON.stringify({
